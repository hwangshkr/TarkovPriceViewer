...
- **Pre‑release** examples: `2.0-beta.1`, `2.0-rc.1`, etc.  
  Generated tag: `v2.0-beta.1`.
- **Stable** examples: `2.0`, `2.0.1`, etc.  
  Generated tag: `v2.0`, `v2.0.1`.
- Workflow rules:
...
1. Edit `TarkovPriceViewer.csproj` and set a version **with a suffix**:
   - Example: `<Version>2.0-beta.1</Version>`
...
4. The workflow will:
   - Read the version (`2.0-beta.1`).
   - Ensure the branch is **not** `main`.
   - Compute the tag `v2.0-beta.1`.
...
1. On `main`, edit `TarkovPriceViewer.csproj` and set a version **without a suffix**:
   - Example: `<Version>2.0</Version>`
2. Commit and push to `main`.
3. On GitHub:
   - Go to **Actions** → workflow **.NET Desktop**.
   - Run the workflow selecting **main**.
4. The workflow will:
   - Read the version (`2.0`).
   - Ensure the branch is `main`.
   - Compute the tag `v2.0`.
...
- If `<Version>` does **not** contain a pre-release suffix (e.g. `2.0`), it does nothing.
- If `<Version>` **does** contain a pre-release suffix (e.g. `2.0-beta.1`):
  - It derives the base version: `2.0`.
  - It replaces `<Version>2.0-beta.1</Version>` with `<Version>2.0</Version>` in `TarkovPriceViewer.csproj`.
  - It commits and pushes the bump: `chore: bump version 2.0-beta.1 -> 2.0`.

Typical flow for a stable release:

1. PR uses a pre-release version, for example `<Version>2.0-beta.1</Version>`.
2. PR is merged into `main` (now `main` has `2.0-beta.1`).
3. The **Auto bump** workflow runs on push to `main` and bumps it to `<Version>2.0</Version>`.
4. When you decide to publish, run the **.NET Desktop** workflow on `main` to create the `v2.0` stable release.
